...
function groupResults(results: DomainResult[]): DomainGroup[] {
  ...
  for (const [domain, domainResults] of byDomain) {
    ...
    const hostnames = new Set(domainResults.map((r) => new URL(r.url).hostname))
    const singleHost = hostnames.size === 1

    const subGroupMap = new Map<string, Array<{ result: DomainResult; displayLabel: string }>>()
    ...
    for (const r of domainResults) {
      ...
      if (singleHost) {
        ...
        const base = domain
        subKey = hostname.slice(0, hostname.length - base.length - 1) || hostname
